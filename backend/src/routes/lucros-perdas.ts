...
function getDataInicio(periodo: string, dataInicio?: string, dataFim?: string): string | null {
  const now = new Date();
  switch (periodo) {
    case "ultimos_7_dias": {
...
function getDataFim(periodo: string, dataFim?: string): string | null {
  if (periodo === "personalizado" && dataFim) return dataFim;
...
app.get("/", (c) => {
  const periodo = c.req.query("periodo") || "todo_periodo";
  ...
  if (dataInicio && dataFim) {
    partidas = db
      .query<{ id: string; data: string; buy_in: number }>(
  ...
  for (const partida of partidas) {
    const participantes = db
      .query<{ jogador_id: string; nome: string; chips_finais: number }>(
